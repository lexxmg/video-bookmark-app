...
// --- ФУНКЦИЯ ИНИЦИАЛИЗАЦИИ ОЧЕРЕДИ ГЕНЕРАЦИИ КАДРОВ ---
function initThumbnailQueue(videoDataArray) {
    if (!videoDataArray || videoDataArray.length === 0) return;

    const queue = [];

    // Наполняем очередь элементами со страницы на основе переданных данных
    videoDataArray.forEach(v => {
        const videoEl = document.getElementById('thumb-video-' + v.id);
        const canvasEl = document.getElementById('canvas-' + v.id);
        const loaderEl = document.getElementById('loader-' + v.id);

        if (videoEl && canvasEl && loaderEl) {
            queue.push({
                id: v.id,
                video: videoEl,
                canvas: canvasEl,
    ...
    function processNext() {
        ...
        current.video.onseeked = function() {
            // Оставляем нашу стабильную задержку в 150мс для тяжелых файлов
            setTimeout(() => {
                try {
